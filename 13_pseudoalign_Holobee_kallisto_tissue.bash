...
# Tasks: Pseudoalign mRNA-seq reads to Holobee sequences using Kallisto. 
#-------------------------------------------------------------------------------
# Inputs:
# Raw mRNA-seq files. Holobee sequence index for Kallisto.
#
# Outputs:
...
mkdir ../02_outputs/${NUMBER}_${1}/20_kallisto_pseudoalignment_holobee_abundances

mkdir ../02_outputs/${NUMBER}_${1}/21_kallisto_pseudoalignment_holobee_summaries

# STEP 3: PSEUDOALIGN OVARY READS USING KALLISTO ----
...
kallisto quant -i ../01_fasta/Holobee_v2016_1_index.idx -o ../../02_outputs/${NUMBER}_${1}/20_kallisto_pseudoalignment_holobee_abundances/${OUTPUT} --rf-stranded ${FILE} ${FILEHANDLE}_R2.fastq.gz 2> ../../02_outputs/${NUMBER}_${1}/21_kallisto_pseudoalignment_holobee_summaries/${OUTPUT}.txt
done
...
cd ../../02_outputs/${NUMBER}_${1}/21_kallisto_pseudoalignment_holobee_summaries/

# Run MultiQC ----
# -n fileName: call report fileName rather than default of multiqc_report.html
# -o: save report in directory 

multiqc . -n 13_NER0008751_obj1_${1}_holobee_kallisto_multiqc_report.html -o ../02_quality_control_reports
...
echo -e "sample\ttreatment\ttime_point\tlane" > ${1}_holobee_study_design.txt
...
echo -e "${SAMPLE}\t${TREATMENT}\t${TIMEPOINT}\t${LANE}" >> ${1}_holobee_study_design.txt
done
